...
public class BlogPostController {
	...
	public ResponseEntity<?> reject(@RequestBody BlogPost blog, @PathVariable String rejectionReason,
			HttpSession session) {
		System.out.println("reason in controller "+rejectionReason);
		System.out.println("blog in controller "+blog.getBlogTitle()+" "+blog.getBlogContent());
		String email = (String) session.getAttribute("loginId");
		if (email == null) {
			ErrorClass error = new ErrorClass(5, "Unauthorised access...");
			return new ResponseEntity<ErrorClass>(error, HttpStatus.UNAUTHORIZED);
		...
		blog=blogPostDao.getBlog(blog.getId());
